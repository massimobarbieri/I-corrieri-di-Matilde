...
\DndDropCapLine{D}{opo l'udienza con Matilde} i PG si recheranno al Castello di Rossena, roccaforte difensiva dell'area per incontrare Goffredo il Gobbo, \textbf{nobile}.

Nel castello saranno presenti 20 guardie, 10 unità di personale di servizio.

Goffredo sarà seccato dall'arrivo dei PG, ma soprattutto alterato poiché è stato rifiutato da Matilde nonostante le abbia promesso terre ed eserciti. Matilde lo ha cacciato da Canossa, ma i consiglieri di Matilde, per non creare scandalo con la cacciata del Duca Goffredo, hanno deciso di farlo alloggiare a Rossena in attesa della festa di fine anno a cui non si sa se parteciperà, ma soprattutto in attesa del suo viaggio di ritorno i Lorena.

\section{Castello di Rossena}

\DndArea{Rossena piano 1}
...
\DndSubArea{Sala del trono} %a
Le pareti di questa sala sono decorate con arazzi e dipinti che rappresentano scene di caccia e ritratti di nobili antenati alla casata dei Canossa. La sala è spoglia, probabilmente destinata a ricevere numerose persone davanti al regnante.

In Questa sala sono presenti 5 \textbf{guardie}. Qui le guardie si riuniscono per decidere le strategie di comando e gestione della fortezza.

\DndSubArea{Ingresso} %b
Sala d'attesa per i visitatori hanno necessità di essere ricevuti dal regnante. Nella stanza è presente un tappeto con un tavolo, una libreria che contiene volumi inerenti battaglie e discendenze nobiliari dei Canossa.
Nella stanza sono anche presenti 2 \textbf{guardie}.

\DndSubArea{Dispensa} %c
In questa dispensa sono presenti 2 barili di vino e un piccolo barile di aceto. Nelle scaffalature sono presenti cassette di verdure, formaggi e salumi stagionati. Tutto il cibo è ottimo da mangiare.

\DndSubArea{Cucina} %d
In questa stanza, o nella dispensa adiacente, sono presenti 4 cuochi (\textbf{popolani}) impegnati a preparare il cibo per il pasto successivo. Ad esclusione della notte è sempre presente qualcuno che sta cucinando. Nella cucina sono presenti padelle, coltelli un acquaio e tutto quello che si può trovare in una cucina del Medioevo.
...
\DndSubArea{Ripostiglio}
Stanza nella quale sono accumulate cianfrusaglie.

\DndSubArea{Camera}
La camera di Goffredo ha un letto in legno, davanti ad esso una pelle di orso funge da tappeto. Un armadio è in realtà un passaggio segreto che dall'interno permette di passare alla stanza 1c. Nella stessa stanza è presente una latrina che scarica direttamente fuori, una sedia e un'armatura in metallo. Goffredo il Gobbo (\textbf{nobile}), vorrà coricarsi spesso in questa stanza e utilizzerà la latrina presente nella stanza stessa.
...
\DndSubArea{Stanza dei ricevimenti}
In questa stanza è presente un tavolo utilizzato per le riunioni e un armadio che contiene un passaggio segreto che conduce nella camera di Goffredo.

\DndSubArea{Ripostiglio}

\DndSubArea{Stanza del camino}

\DndSubArea{Stanza da lettura}
